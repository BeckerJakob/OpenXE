...
/*
  ...
  protected function CreateRechnungTeilStornoGutschrift($rechnung, $positions)
  {
    ...
    $arr = $this->app->DB->SelectRow("SELECT NOW() as datum,IFNULL(NULLIF((SELECT a.belegnr FROM auftrag AS a WHERE a.id = `rechnung`.`auftragid` AND a.belegnr <> '0' LIMIT 1), ''), ihrebestellnummer) AS ihrebestellnummer,projekt, belegnr as rechnung,anschreiben,aktion,ansprechpartner,titel,lieferbedingung,bodyzusatz,
